...
p_dog_train_feat = '../trans_data/dog.simple1'
p_dog_valid_feat = '../trans_data/valid.simple1'
p_dog_valid_id = '../trans_data/valid.txt'

p_pig_train_feat = '../trans_data/train.simple1'
p_pig_valid_feat = '../trans_data/test.simple1'
p_pig_valid_id = '../raw_data/test.txt'

p_label_map = '../dataset/label_map_dog'
  ...
  for line in open(p_in):
    ...
    for pair in session_queries.split(';'):
        ...
        for i, word in enumerate(feat_list):
            ...
            word_df[word] = word_df.get(word, 0) + 1
            if i>=1:
                word = 'sq_' + ' '.join(feat_list[i-1:i+1])
                word_df[word] = word_df.get(word, 0) + 1
    ...
    for pair in session_titles.split(';'):
        ...
        for i, word in enumerate(feat_list):
            ...
            word_df[word] = word_df.get(word, 0) + 1
            if i>=1:
                word = 'st_' + ' '.join(feat_list[i-1:i+1])
                word_df[word] = word_df.get(word, 0) + 1
  ...
  for line in open(p_in):
    ...
    for pair in titles.split(';'):
        ...
        for i, word in enumerate(feat_list):
            ...
            word = feat_list[i]
            '''
            if word in word_map and word_map[word] in word_tf:
                word = 'qt_' + word
...
save_test('../trans_data/cat.txt', '../dataset/cat_test')
save_test('../trans_data/valid2.txt', '../dataset/dog_test')
