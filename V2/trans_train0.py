...
s_train = '../trans_data/train.simple0'
s_test = '../trans_data/test.simple0'
s_dog = '../trans_data/dog.simple0'
s_valid = '../trans_data/valid.simple0'
...
def trans(in1, in2, out1, out2):
    ...
    for line in open(in1):
        ...
        if title and title != '-':
            session_click[title] = 1
        session_query[query] = 1
        ...
        if label.startswith('CLASS=TEST'):
            ...
            session_train_query[query] = ''.join(sorted(label.split(' | '))) 
    ...
    with open(out1, 'w') as ft:
        for query in train_dict:
            ...
            title_pairs = ['%s:%s' % (v[0], v[1]) for v in titles[:]]
